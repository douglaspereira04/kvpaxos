...
template<typename T>
std::vector<T> LMDBStorage<T>::scan(const T &key, size_t len) {
    std::vector<T> result;
    MDB_txn* __txn;
    MDB_cursor* cursor;
    int rc;

    rc = mdb_txn_begin(__env, nullptr, MDB_RDONLY, &__txn);
    if (rc != 0) {
        abort();
    }
    ...
    while (rc == MDB_SUCCESS && count < len) {
        std::string found_value(reinterpret_cast<char*>(__value.mv_data), __value.mv_size);
