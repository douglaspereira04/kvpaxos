...
DoubleRandFunction uniform_double_distribution_rand(double min_value, double max_value, long seed) {
    std::mt19937 generator(seed);
...
RandFunction binomial_distribution(
    int n_experiments, double success_probability, long seed
) {
    std::mt19937 generator(seed);
...
RandFunction skewed_latest_distribution(acknowledged_counter<long> *&counter, zipfian_int_distribution<long> *& zip, long seed){
    std::mt19937 generator(seed);
...
RandFunction ranged_binomial_distribution(
    int min_value, int n_experiments, double success_probability, long seed
) {
    auto random_func = binomial_distribution(
        n_experiments - min_value, success_probability, seed
    );
