                ...
                if [ "$v_" = "rep" ]; then
                    for track_length_ in "${track_length[@]}"; do
                        for q_size_ in "${q_size[@]}"; do
                            ...
                            echo ./${v_}_${track_length_}_${q_size_} ${requests[$i]}  $p_  $n_initial_keys  ${deltat[$i]}  $method  ${workloads[$i]}_requests.txt  0  $arrival_rate_seed  $deltah $c_
                            ./${v_}_${track_length_}_${q_size_} ${requests[$i]}  $p_  $n_initial_keys  ${deltat[$i]}  $method  ${workloads[$i]}_requests.txt  0  $arrival_rate_seed  $deltah $c_ > output/$file_name
                            if [ $? -ne 0 ]; then
                                echo "ERROR"
                            fi
