...
deltat=(100000 100000 10000)
requests=(1000000 1000000 100000)
        ...
        if [ "$version" = "rep" ]; then
            for track_length_ in "${track_length[@]}"; do
                for q_size_ in "${q_size[@]}"; do
                    ...
                    echo ./${version}_${track_length_}_${q_size_} ${requests[$i]}  $p_  $n_initial_keys  ${deltat[$i]}  $method  ${workloads[$i]}_requests.txt  0  $arrival_rate_seed  $deltah
            ...
            echo ./${version} ${requests[$i]}  $p_  $n_initial_keys  0  $method  ${workloads[$i]}_requests.txt  0  $arrival_rate_seed  0
